...
#define ROUTINE_STATE_IDLE                    0x00U
...
static const uint8 k_dtcs[] = {
    0xC1U, 0x03U, 0x00U, 0x09U,  /* C10300 - CAN bus off */
    0xC2U, 0x01U, 0x00U, 0x04U,  /* C20100 - motor overcurrent */
    0xC4U, 0x01U, 0x00U, 0x09U,  /* C40100 - E-stop activated */
};
...
{
    uint8 payload[DID_RESPONSE_MAX_BYTES];
    if (n + 2U > DID_RESPONSE_MAX_BYTES) { return 0U; }
...
static void send_routine_positive(uint8 subf, uint16 routine_id, const uint8 *bytes, uint32 n)
{
    uint8 payload[8];
    payload[0] = subf;
    payload[1] = (uint8)(routine_id >> 8U);
    payload[2] = (uint8)(routine_id & 0xFFU);
    for (uint32 i = 0U; i < n; i++) { payload[3U + i] = bytes[i]; }
    (void)send_positive(0x31U, payload, 3U + n);
}
...
static void svc_read_did(const uint8 *req, uint32 len)
{
    ...
    switch (did) {
    case 0xF190U: {
        (void)send_did(0xF190U, g_vin, VIN_LENGTH);
        return;
    }
    case 0xF187U: {
        /* SupplierSWNumber */
        static const uint8 sw[8] = { 'C', 'V', 'C', '-', 'U', 'D', 'S', '1' };
        (void)send_did(0xF187U, sw, 8U);
        return;
    }
    case 0xF195U: {
        /* SystemSupplierECUHWVersionNumber */
        static const uint8 hw[8] = { 'G', '4', '7', '4', 'R', 'E', 'A', '1' };
        (void)send_did(0xF195U, hw, 8U);
        return;
    }
    case 0xF186U: {
...
static void svc_routine_control(const uint8 *req, uint32 len)
{
    ...
    switch (subf) {
    case 0x01U:
        g_motor_self_test_state = ROUTINE_STATE_RUNNING;
        routine_state = g_motor_self_test_state;
        send_routine_positive(subf, routine_id, &routine_state, 1U);
        return;
    case 0x02U:
        g_motor_self_test_state = ROUTINE_STATE_IDLE;
        routine_state = g_motor_self_test_state;
        ...
        return;
    case 0x03U:
        if (g_motor_self_test_state == ROUTINE_STATE_RUNNING) {
            g_motor_self_test_state = ROUTINE_STATE_PASSED;
        }
        routine_state = g_motor_self_test_state;
        send_routine_positive(subf, routine_id, &routine_state, 1U);
        return;
    default:
        (void)send_negative(0x31U, NRC_SUBFUNCTION_NOT_SUPPORTED);
        return;
    }
}
