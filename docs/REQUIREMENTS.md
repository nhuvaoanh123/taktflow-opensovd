...
- Acceptance criteria:
  1. With 7 backends (3 virtual, 4 physical via CDA) and each holding 1-5
     DTCs, the response is the ordered concatenation tagged by component id.
  2. Killing one virtual ECU mid-call still returns the other 6 ECUs' data
     with a warning log entry identifying the missing backend.
  3. Killing all 7 backends returns HTTP 502 with `BackendFailure`.
...
- Acceptance criteria:
  1. Returns 7 entries for the Taktflow MVP (CVC, FZC, RZC, BCM, ICU, TCU, SC).
...
- Description: Virtual ECUs (CVC, FZC, RZC as POSIX builds; BCM, ICU, TCU as
  additional POSIX containers) SHALL accept DoIP on TCP 13400 and respond to
  UDS 0x19 / 0x14 / 0x31 issued by CDA.
...
- Acceptance criteria:
  ...
  2. MDDs for all 7 ECUs are generated by `odx-converter` from Taktflow ODX
     files and committed.
...
- Description: GET `/sovd/v1/components/{id}/faults` MUST return in <= 500 ms
  at P99 across all 7 ECUs, measured on the production Pi topology under
  nominal load.
...
#### NFR-5.1 7-ECU MVP
- Description: The system MUST handle the 7-ECU Taktflow topology
  (BCM/ICU/TCU virtual + CVC/FZC/RZC/SC physical) concurrently.
- Rationale: MASTER-PLAN §1.1.
- Acceptance criteria: HIL scenario `hil_sovd_01_read_dtcs_all` exercises all 7.
...
#### COMP-1.1 ISO 17978 SOVD conformance (MVP subset)
- Description: The SOVD Server and Gateway MUST conform to ISO 17978 for the
  subset of resources specified in upstream mvp.md §Use-cases. Non-subset
  endpoints MAY return 501.
- Rationale: upstream mvp.md; MASTER-PLAN §B.1.
- Acceptance criteria: a conformance test suite (OpenAPI-schema-driven) runs
  against the Server in SIL.
- Source: ISO 17978; upstream mvp.md.
- Phase: 4
